...
export class SolicitudCardComponent implements OnInit {
  ...
  eliminaSolicitud(){ this.eliminaSolicitudPri(); }
  private eliminaSolicitudPri(){
    this.solServ.deleteSolicitud(localStorage.getItem("token")!,this.solicitud._id).subscribe({
      next:val=>{
        if(val.status < 220){
          if(this.visible) this.locServ.getString("mensajesInformacion.202rechazaSolicitud")
            .subscribe(val => this.apiRespServ.resMensajeSucBtn(val,true));
          else this.locServ.getString("mensajesInformacion.202deleteSolicitud")
            .subscribe(val => this.apiRespServ.resMensajeSucBtn(val,true));
        }else console.log("Error: ", val.message);
      }, 
